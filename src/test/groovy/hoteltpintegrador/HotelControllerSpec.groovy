...
class HotelControllerSpec extends Specification {
    ...
    void "Test the index action returns the correct model"() {
        given:
        controller.hotelService = Mock(HotelService) {
            1 * list(_) >> []
            1 * count() >> 0
        }
    ...
    void "Test the save action with an invalid instance"() {
        given:
        controller.hotelService = Mock(HotelService) {
            1 * save(_ as Hotel) >> { Hotel hotel ->
                throw new ValidationException("Invalid instance", hotel.errors)
            }
        }
